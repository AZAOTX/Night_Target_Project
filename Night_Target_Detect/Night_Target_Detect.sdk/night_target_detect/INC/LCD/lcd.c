...
void LCD_Init()
{
	...
	memset(frame,0xFF,1280*720*BYTES_PIXEL);
	LCD_Refresh();
}
...
{
	//刷新Data Cache，更新图像
	Xil_DCacheFlushRange((UINTPTR) FrameBuffer_Addr, 1280*720*BYTES_PIXEL);
//	Xil_DCacheFlushRange((UINTPTR) FrameBuffer_Addr+1280*720*BYTES_PIXEL, 1280*720*BYTES_PIXEL);
//	Xil_DCacheFlushRange((UINTPTR) FrameBuffer_Addr+1280*720*BYTES_PIXEL+1280*720*BYTES_PIXEL, 1280*720*BYTES_PIXEL);
}
...
void LCD_DrawPoint(uint16_t x,uint16_t y,uint32_t color)
{
	...
	if(y>720)
		y=720;
	frame[BYTES_PIXEL*x + 1280*BYTES_PIXEL*y] = color&0x0000ff;
	frame[BYTES_PIXEL*x + 720*BYTES_PIXEL*y + 1] = (color>>8)&0x0000ff;
...
void LCD_Color_Fill(uint16_t x1,uint16_t y1,uint16_t x2,uint16_t y2,uint32_t color)
{
	...
	for(y=y1;y<=y2;y++)
	{
		...
		{
			frame[BYTES_PIXEL*x + 1280*BYTES_PIXEL*y] = color&0x0000ff;
			frame[BYTES_PIXEL*x + 1280*BYTES_PIXEL*y + 1] = (color>>8)&0x0000ff;
		}
	}
...
void LCD_Clear(uint32_t color)
{
	...
	for(y=0;y<720;y++)
	{
		...
		{
			frame[BYTES_PIXEL*x + 1280*BYTES_PIXEL*y] = color&0x0000ff;
			frame[BYTES_PIXEL*x + 12880*BYTES_PIXEL*y + 1] = (color>>8)&0x0000ff;
		}
	}
...
void LCD_DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,uint32_t color)
{
	...
	for(t=0;t<=distance+1;t++ )    //画线输出
	{
		frame[BYTES_PIXEL*uRow + 1280*BYTES_PIXEL*uCol] = color&0x0000ff;
...
void LCD_ShowChar(uint16_t x,uint16_t y,uint8_t num,uint8_t size,uint8_t mode)
{
    ...
    for(t=0;t<csize;t++) {
        ...
        for(t1=0;t1<8;t1++) {
            if(temp&0x80)
			{
            	frame[BYTES_PIXEL*x + 1280*BYTES_PIXEL*y] = POINT_COLOR&0x0000ff;
            	frame[BYTES_PIXEL*x + 1280*BYTES_PIXEL*y + 1] = (POINT_COLOR>>8)&0x0000ff;
			}
            else if(mode==0)
            {
				frame[BYTES_PIXEL*x + 1280*BYTES_PIXEL*y] = BACK_COLOR&0x0000ff;
				frame[BYTES_PIXEL*x + 1280*BYTES_PIXEL*y + 1] = (BACK_COLOR>>8)&0x0000ff;
            }
            temp<<=1;
            y++;
            if(y>=720)
...
void LCD_DisplayPic(uint16_t x, uint16_t y,uint8_t const *Pic)
{
	...
	for(y_pos=y;y_pos<y+height;y_pos++)
	{
		...
		{
			frame[BYTES_PIXEL*x_pos + 1280*BYTES_PIXEL*y_pos] =  Pic[Cnt + 8 + 1];
			frame[BYTES_PIXEL*x_pos + 1280*BYTES_PIXEL*y_pos + 1] = Pic[Cnt + 8];
			Cnt+=BYTES_PIXEL;
		}
	}
