...
void OV5640_Init()
{
    ...
    if(ID == 0x5640)  //获取到正确的ID：OV5640
    {
		...
		SCCB_Write(0x3103,0x11); // system clock from pad, bit[1]
		    ...
		    SCCB_Write(0x3a13,0x4F); // pre-gain = 1.047x************************************************0x43
...
//		    SCCB_Write(0x3a0f, 0x40); // 增加稳定范围高值
//		    SCCB_Write(0x3a10, 0x38); // 增加稳定范围低值
//		    SCCB_Write(0x3a1b, 0x40); // 增加输出稳定范围高值
//		    SCCB_Write(0x3a1e, 0x30); // 增加输出稳定范围低值
//		    SCCB_Write(0x3a11, 0x80); // 增加快速区域高值
//		    SCCB_Write(0x3a1f, 0x28); // 增加快速区域低值
		    ...
		    SCCB_Write(0x3820,0x41); // no flip
		    SCCB_Write(0x3821,0x07); // no mirror
...
//
		    ...
		    SCCB_Write(0x3808,0x05); // DVPHO
			SCCB_Write(0x3809,0x00); // DVPHO
			SCCB_Write(0x380a,0x02); // DVPVO
			SCCB_Write(0x380b,0xd0); // DVPHO
...
//		    SCCB_Write(0x3a15,0xe0); // 50Hz max exposure
		    ...
		    SCCB_Write(0x3a02,0x0d); // 60Hz max exposure
			SCCB_Write(0x3a03,0x80); // 60Hz max exposure
			SCCB_Write(0x3a14,0x0d); // 50Hz max exposure
			SCCB_Write(0x3a15,0x80); // 50Hz max exposure
		    ...
		    SCCB_Write(0X3501,0xff);

		    SCCB_Write(0x3a00,0x7c);


		    SCCB_Write(0x4740,0x20); // VS 1
//		    SCCB_Write(0x5587, 0x00); // 提高亮度
//		    SCCB_Write(0x5586, 0x20); // 调整对比度

	}
}

void OV5640_720p_Init()
{
	uint16_t ID = 0;
    ...
    if(ID == 0x5640)  //获取到正确的ID：OV5640
    {
		...
		SCCB_Write(0x3103,0x11); // system clock fSCCB_Write pad, bit(1)
		SCCB_Write(0x3008,0x82); // software reset, bit(7)
		SCCB_Write(0x3008,0x42); // software power down, bit(6)
		SCCB_Write(0x3103,0x03); // system clock fSCCB_Write PLL, bit(1)
		SCCB_Write(0x3017,0xff); // FREX, Vsync, HREF, PCLK, D(9:6) output enable
		SCCB_Write(0x3018,0xff); // D(5:0), GPIO(1:0) output enable
		SCCB_Write(0x3034,0x1a); // MIPI 10-bit
		SCCB_Write(0x3037,0x13); // PLL root divider, bit(4), PLL pre-divider, bit(3:0)
		...
		SCCB_Write(0x3c0b,0x40); // sample number(7:0)
		SCCB_Write(0x3810,0x00); // Timing Hoffset(11:8)
		SCCB_Write(0x3811,0x10); // Timing Hoffset(7:0)
		SCCB_Write(0x3812,0x00); // Timing Voffset(10:8)
		SCCB_Write(0x3813,0xfa); // timing V offset
		SCCB_Write(0x3708,0x64);
		SCCB_Write(0x4001,0x02); // BLC start fSCCB_Write line 2
		SCCB_Write(0x4005,0x1a); // BLC always update
		SCCB_Write(0x3000,0x00); // enable blocks
		SCCB_Write(0x3004,0xff); // enable clocks
		SCCB_Write(0x300e,0x58); // MIPI power down, DVP enable
		SCCB_Write(0x302e,0x00);
		SCCB_Write(0x4300,0x61); // RGB565   //h4300_61
		SCCB_Write(0x501f,0x01); // RGB565
		SCCB_Write(0x440e,0x00);
		SCCB_Write(0x5000,0xa7); // Lenc on, raw gamma on, BPC on, WPC on, CIP on
		// AEC target 自动曝光控制
		...
		SCCB_Write(0x530c,0x06); // CIP sharpen TH offset 2
		SCCB_Write(0x5025,0x00);
		SCCB_Write(0x3008,0x02); // wake up fSCCB_Write standby, bit(6)

		// 12824'h720, 30fps
		// input clock 24Mhz, PCLK 42Mhz
		SCCB_Write(0x3035,0x11); // PLL JPEG mode 11->15fps);21->7.5fps); 41->3.75fps);
		SCCB_Write(0x3036,0x69); // PLL

		SCCB_Write(0x3c07,0x07); // lightmeter 1 threshold(7:0)
		SCCB_Write(0x3820, 0x40);    // flip
		SCCB_Write(0x3821, 0x06); // no mirror
		SCCB_Write(0x3814,0x11); // timing X inc
		SCCB_Write(0x3815,0x11); // timing Y inc
		SCCB_Write(0x3800,0x00); // HS
		SCCB_Write(0x3801,0x00); // HS
		SCCB_Write(0x3802,0x00); // VS
		SCCB_Write(0x3803,0x00); // VS
		...
		SCCB_Write(0x3808,0x05 ); // DVPHO	(<1280>500) (<640>280)IMAGE_WIDTH
		SCCB_Write(0x3809,0x00 ); // DVPHO
		SCCB_Write(0x380a,0x02 ); // DVPVO	(<720>2d0)  (<480>1e0)IMAGE_HEIGHT
		SCCB_Write(0x380b,0xd0 ); // DVPHO

		SCCB_Write(0x380c,0x0b); // HTS
		SCCB_Write(0x380d,0x1c); // HTS
		SCCB_Write(0x380e,0x07); // VTS
		SCCB_Write(0x380f,0xb0); // VTS
		SCCB_Write(0x3813,0x04); // timing V offset
		SCCB_Write(0x3618,0x04);
		SCCB_Write(0x3612,0x2b);
		SCCB_Write(0x3709,0x12);
		SCCB_Write(0x370c,0x00);

		SCCB_Write(0x4004,0x06); // BLC line number
		SCCB_Write(0x3002,0x00); // reset JFIFO, SFIFO, JPG
		SCCB_Write(0x3006,0xff); // disable clock of JPEG2x, JPEG
		SCCB_Write(0x4713,0x03); // JPEG mode 3
		SCCB_Write(0x4407,0x01); // Quantization scale
		SCCB_Write(0x460b,0x35);
		SCCB_Write(0x460c,0x22);
		SCCB_Write(0x4837,0x16); // MIPI global timing
		SCCB_Write(0x3824,0x02); // PCLK manual divider
		SCCB_Write(0x5001,0xa3); // SDE on, CMX on, AWB on

//		//颜色偏黄或偏绿，请修改AWB颜色分量设置
//		SCCB_Write(0x3400,0x04); // R
//		SCCB_Write(0x3401,0xf0); //
//		SCCB_Write(0x3402,0x04); // G
//		SCCB_Write(0x3403,0x10); //
//		SCCB_Write(0x3404,0x04); // B
//		SCCB_Write(0x3405,0x60); //
//		SCCB_Write(0x3406,0x01); // 使能手动设置颜色

		SCCB_Write(0x3503,0x00); // AEC/AGC on
		SCCB_Write(0x4740,0x20); // VS 1   //

//		SCCB_Write(0x3a00,0x7c);
//		SCCB_Write(0x3a02,0x0a); // 60Hz max exposure
//		SCCB_Write(0x3a03,0x80); // 60Hz max exposure
//		SCCB_Write(0x3a14,0x0a); // 50Hz max exposure
//		SCCB_Write(0x3a15,0x80); // 50Hz max exposure
